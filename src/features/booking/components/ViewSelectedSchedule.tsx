...
export const ViewSelectedSchedule = ({mock_schedule, selectedSchedule, setViewFLight} : any) => {
    const {value, setValue} = useMultiForm()
    const handleConfirmSchedule = () => {
        setViewFLight(false)
        setValue((prev : any) => ({...prev, data:{...prev.data, schedule_id:selectedSchedule }}))
    }
    console.log(value);
  ...
  return (
    <div className='fixed top-0 left-0 h-screen w-full bg-gray-500 bg-opacity-75 place-content-center place-items-center z-50 '>
        <div className='relative min-w-[800px] bg-bg rounded-lg p-4 animate-topToBottom'>
            <div >
                {mock_schedule
                .filter((schedule : any) => (schedule.id === selectedSchedule))
                .map((schedule : any) => 
                    
                <div className=' gap-4 flex' key={schedule.id}>
                        ...
                        <div className='flex'>
                            <div className='w-full flex flex-col'>
                                ...
                                <div className='flex gap-2'>
                                    <Typography variant='info'>{schedule.date}</Typography>
                                    <Typography variant='info'>{schedule.departure}</Typography>
                                </div>
                            </div>
                            <div className='w-full flex flex-col'>
                                ...
                                <div className='flex gap-2'>
                                    <Typography variant='info'>{schedule.date}</Typography>
                                    <Typography variant='info'>{schedule.arrival}</Typography>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='w-2/4 px-5 py-4 rounded-md bg-white'>
                        ...
                        <Typography variant='info' className='flex justify-between'>Base Amount <span>{schedule.fare}</span></Typography>
                        <Typography variant='info' className='flex justify-between'>Additional Fee <span>₱0.00</span></Typography>
                        <Typography variant='info' className='flex justify-between'>Subtotal <span>{schedule.fare}</span></Typography>
                        <Typography variant='info' className='flex justify-between'>Amount Off <span>₱0.00</span></Typography>
                        <hr className='my-4'/>
                        <Typography variant='body2' className='font-bold flex justify-between'>Total Amount : <span>{schedule.fare}</span></Typography>
                    </div>
                </div>
            )}
